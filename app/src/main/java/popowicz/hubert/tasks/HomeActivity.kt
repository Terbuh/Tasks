...
import popowicz.hubert.tasks.util.StorageOperations
...
class HomeActivity : ComponentActivity() {
    ...
    fun TaskListView() {
        ...
        Column(modifier = Modifier.padding(horizontal = 20.dp)) {
            Text(
                text = "Task list", fontSize = 30.sp, modifier = Modifier.padding(top = 20.dp)
            )
            LazyColumn(
    ...
    fun HomeView() {
        ...
        Box(
            ...
            when (taskViewModel.getAllTasksStatus) {
                ...
                TaskOperationStatus.LOADING -> CircularProgressIndicator(
                    Modifier.align(
                        Alignment.Center
                    )
                )
